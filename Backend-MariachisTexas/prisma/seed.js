...
async function main() {
    ...
    const getPermiso = (nombre) => todosLosPermisos.find(p => p.nombre === nombre);
    // ─── MÓDULOS POR ROL ─────────────────────────────────────
    ...
    // ─── ASIGNAR ─────────────────────────────────────────────
    const asignar = async (rolId, modulos) => {
    ...
    await prisma.usuario.create({
        data: {
            ...
            password: await bcryptjs_1.default.hash('Admin-123456', 10),
            rolId: admin.id
        }
    });
    await prisma.usuario.upsert({
        ...
        create: {
            ...
            password: await bcryptjs_1.default.hash('Empleado-123456', 10),
            rolId: empleado.id
        }
    });
    // ─── EMPLEADOS DE PRUEBA ───────────────────────────────────
    ...
    const directaUser = await prisma.usuario.upsert({
        ...
        create: {
            ...
            password: await bcryptjs_1.default.hash('Directa-123456', 10),
            rolId: cliente.id
        }
    });
    ...
    const clientePruebaUser = await prisma.usuario.upsert({
        ...
        create: {
            ...
            password: await bcryptjs_1.default.hash('Cliente-123456', 10),
            rolId: cliente.id
        }
    });
    await prisma.cliente.upsert({
