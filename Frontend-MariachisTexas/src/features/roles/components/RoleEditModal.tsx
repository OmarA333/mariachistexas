...
const validateRole = (data: any): RoleFormErrors => {
...
export const RoleEditModal: React.FC<Props> = ({ isOpen, onClose, onSave, role }) => {
  ...
  const handlePermissionToggle = (moduleId: string) => {
    setFormData((prev: any) => {
      ...
      return {
        ...prev,
        permissions: exists
          ? prev.permissions.filter((permission: string) => permission !== moduleId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = getErrorMessage(error, 'Error al actualizar el rol.');
