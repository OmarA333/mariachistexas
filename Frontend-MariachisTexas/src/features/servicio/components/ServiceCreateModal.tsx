...
const EMPTY_FORM: Omit<Service, 'id' | 'estado'> = {
  nombre: '',
  descripcion: '',
  precio: 0,
};
...
export const ServiceCreateModal: React.FC<Props> = ({ isOpen, onClose, onSave }) => {
  ...
  const [formData, setFormData] = useState<Omit<Service, 'id' | 'estado'>>(EMPTY_FORM);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Solo cerramos y limpiamos si el guardado fue exitoso
      handleClose();
    } catch (err: any) {
      ...
      // El modal se queda abierto con los datos intactos
    } finally {
      setLoading(false);
    }
  };
