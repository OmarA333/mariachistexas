...
// Ajusta las keys a los campos reales de tu ClientForm
...
export const ClientEditModal: React.FC<Props> = ({ isOpen, onClose, onSave, client }) => {
  ...
  // Cargar datos del cliente cuando abre el modal
  useEffect(() => {
    if (client && isOpen) {
      setFormData(client);
      setError(null);
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-fade-in-up overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium tracking-wide mt-0.5">
                Actualizar información de {client?.name}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
          ...
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed text-white px-8 py-3 rounded-xl text-xs font-bold tracking-widest uppercase flex items-center gap-2 shadow-lg shadow-emerald-900/20 hover:shadow-emerald-900/30 transition-all transform hover:-translate-y-0.5"
          >
            {photo.uploading
